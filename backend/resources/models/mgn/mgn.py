...
class MGN(nn.Module):
    def __init__(self, args):
        ...
        #模型的第一部分，包括ResNet的初始卷积层、池化层和一部分残差块
        ...
        #ResNet中的第四个阶段（resnet.layer3）的一部分，包括多个残差块
        res_conv4 = nn.Sequential(*resnet.layer3[1:])

        #ResNet中的第五个阶段（resnet.layer4），代表模型的主干分支
        res_g_conv5 = resnet.layer4 # 主干分支 Global Branch

        #定义了res_p_conv5，它是一个卷积层的序列，将主干分支的特征进行降维
        ...
        #res_p_conv5权重被加载为与主干分支 Global Branch相同
        res_p_conv5.load_state_dict(resnet.layer4.state_dict())

        #主干分支 Global Branch 提取全局特征
    ...
    def forward(self, x):
        ...
        #zp2 和zp3 进行不同尺寸的最大池化，提取各个部分的特征
